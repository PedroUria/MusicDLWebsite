...
def ltsm_gen(net, seq_len, file_name, notes_encoded, n_steps=100, hidden_size=178,
    ...
    for nt in x:  # To start predicting. This will be later removed from
        notes.append(nt.numpy())  # the final output
    h_state = torch.zeros(1, x.shape[1], hidden_size).float()
    c_state = torch.zeros(1, x.shape[1], hidden_size).float()
    ...
    for _ in range(n_steps):
        ...
        y_pred = y_pred[:, -1, :].data  # We only care about the last predicted note
