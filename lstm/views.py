...
def author_generate(request, pk, trained_lstm_selected):
    try:
        ...
        now = datetime.now()
        output_file_name = os.getcwd() + "/static/lstm/generated_samples/" + trained_lstm_selected.replace(".pkl", "") + "_generated_" + datetime.strftime(now, "%Y-%m-%d_%H:%M:%S.%f")
        for file in current_midi_and_wav_files:
            file_timestamp = datetime.strptime(file[file.find("_generated_")+11:-4], "%Y-%m-%d_%H:%M:%S.%f")
            if divmod((now - file_timestamp).seconds, 60)[0] > 10:  # Removes files older than 10 minutes
        ...
        error_msg = str(e)
        for get_m in ["submitted", "thres_note_selected_right", "thres_note_selected_left", "thres_hold_selected_right",
                      "thres_hold_selected_left", "seq_len_selected", "n_steps_selected", "tempo_selected"]:
            if get_m in error_msg:
                error_msg = ""
                break
    ...
    return render(request, 'author_generate.html',
                  {"author": author, "trained_lstm_selected": trained_lstm_selected,
                   "output_wav": output_file_name[output_file_name.find("generated_samples")+18:] + ".wav",
                   "input_notes_seq": input_notes_seq, "clear": clear,
                   "output_mid_name": output_file_name[output_file_name.find("generated_samples")+18:] + ".mid", "error_msg": error_msg},)
